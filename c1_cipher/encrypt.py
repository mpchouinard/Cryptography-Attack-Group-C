...
def text_to_binary_list(file_path):
    binary_list = []
    ...
    for char in text:
        binary_str = format(ord(char), '08b')  # 8-bit binary
        binary_list.extend(int(bit) for bit in binary_str)

    return binary_list

def write_binary_list(file_path, binary_list):
...
def main():
    ...
    for i in range(textlen):
        ctext.append(ptext[i]^(stream_0[i] ^ (stream_1[i] & stream_2[i])))

    write_binary_list("./ctext.txt", ctext)
